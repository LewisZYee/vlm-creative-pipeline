"""
Creative Production Pipeline — Conversational Agent UI (Long-form)
Run: streamlit run agent-pipeline/app_long.py
...
from pipeline.storyboard_long import generate_storyboard_short
from pipeline.shotprompter_long import generate_shot_prompts_short
...
with st.sidebar:
    ...
    st.divider()
    st.caption("Lewis Zhao - BytePlus EUI Solution Architect")
...
with st.chat_message("assistant", avatar="🎬"):
    ...
    if st.session_state.current_step == 0:
        ...
        st.markdown(
            ...
            "score it across 6 dimensions, then generate an improved version."
        )
...
if st.session_state.current_step >= 1 and st.session_state.critique_result:
    ...
    if st.session_state.storyboard_result is None:
        ...
        with st.chat_message("assistant", avatar="🎬"):
            st.markdown(
                f"I can generate an improved **storyboard** that "
    ...
    if st.session_state.storyboard_result:
        ...
        with st.chat_message("assistant", avatar="🎬"):
            st.markdown(
                ...
                "flow tightened and compliance issues fixed."
            )
    ...
    if st.session_state.shot_result:
        ...
        with st.chat_message("assistant", avatar="🎬"):
            ...
            for shot in shots:
                ...
                _vk        = f"shot_prompt_{sid}"
                _vr        = f"shot_{sid}"

                # Initialise editable prompt once
                if _vk not in st.session_state:
                    st.session_state[_vk] = shot.get("prompt", "")

                st.markdown(f"**Shot {sid} — {label}** · {time_range} ({dur}s)")
                st.text_area(
                    f"Prompt (editable) — Shot {sid}",
                    height=220,
                    key=_vk,
                    label_visibility="collapsed",
                )
                prompt     = st.session_state[_vk]
                vid_result = st.session_state.gen_videos.get(_vr, {})
                ...
                if is_done and vid_result.get("video_url"):
                    st.success(f"Shot {sid} generated!")
                    ...
                    st.error(f"Shot {sid} failed: {vid_result.get('error', 'unknown')}")

                st.markdown("")  # spacing between shots
